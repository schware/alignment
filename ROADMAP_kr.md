...
- **방향 정리, 2026-09-10** (논의만, 구현 없음): Netty Device Server —
  "POS SERVER" — 를 BO에 합칠 것인가. 소유자의 처음 감각은 BO와 OMS는
  확실히 갈리지만 BO와 POS SERVER는 합쳐도 될 것 같다는 쪽이었다.
  결론은 셋 유지이고, 결정적인 이유는 **노출 범위**다 — BO는 평문 HTTP
  관리 콘솔이라 LAN 전용으로 두기로 했는데 단말은 인터넷에서 붙는다.
  한 프로세스면 정책도 하나가 되고, TLS가 생기기 전엔 둘 다 못 받는다.
  그 뒤로 확장 축이 다르다는 것(운영자 수 vs 단말 수), BO가 지금
  인스턴스 하나만 뜬다는 것이 따라온다. 정작 합치고 싶게 만든 건
  데이터 접근 문제였다 — POS SERVER가 BO의 장비 목록을 못 본다 —
  그건 BO 테이블 읽기 권한으로 합치지 않고 풀린다. 나온 모양은
  **BO는 보여주고 명령한다, 상태는 POS SERVER가 갖는다**(제어면/실행면)
  이고, BO의 기존 3단계 권한이 하나의 콘솔을 기준정보/운영·정산/
  헬프데스크 세 얼굴로 나눠준다. 솔직히 적어둘 부분: 단말 서버를
  분리하던 *원래* 이유(연결 유지가 비쌌다)는 이제 거의 사라졌다
  (virtual thread, 브로커). 경계는 만들어진 이유와 다른 이유로 살아남은
  것이고, 이 질문을 다시 열 조건은 TLS다.
- **무엇, 2026-09-10**: 소유자가 전체 흐름의 설계서를 넘겼다 — 주문
  채널, 딜리버리 채널, KDS, DID, 그리고
  `(매출일자, 매장, 단말기번호, 거래번호)`를 키로 하고 수락 시점에
  거래번호를 채번하는 매출 모델. 빠져 있던 토대 두 개를 만들었다.
  **BO의 매장 마스터** — 매장이라는 게 어디에도 없었다. `store-01`은
  서비스마다 흩어진 문자열이었고 주문 채널에는 코드에 박힌 10개 목록이
  전부였다. 그리고 **Order의 영업일** — 개점/마감, 주문마다 그때 열려
  있던 영업일자를 찍고, 개점 안 한 매장의 주문은 거절한다. 판단 두 개는
  umbrella의 ADR-0006에 남겼다: 영업 상태가 BO가 아니라 Order에 있는
  이유는 그게 쓰기 경로에서 읽히기 때문이고(매출 한 건 안에 서비스 간
  호출을 넣는 건 잘못된 모양이다), **타이머로 마감하지 않는** 이유는
  00:30에 영업 중인 가게는 하루를 안 끝낸 것이기 때문이다 — 날짜는
  사람이 마감을 누르거나, 지나간 영업일에 개점을 누를 때 넘어간다.
  이걸로 예전의 "POS WebSocket이 붙어 있나로 매장 닫힘을 추측하는"
  발상도 폐기됐다. 남은 것: `Order_header/Detail/Pay/Etc` + `Tr_*`
  재구조화(이 서비스에 대한 ADR-0001의 JSONB 선택을 뒤집는다),
  거래번호 자체, 회원번호 규칙(날짜+순번+CRC), 그리고 단말 화면 3종 —
  KDS와 DID는 저장소만 있고 비어 있다.
...
- **위 작업을 실물로 확인하다가 발견하고 고친 버그, 2026-09-13**: Order
  컨테이너에 `EVENTS_REDIS=true`가 없어서, 로그만 남기는
  `InMemoryEventPublisher`로 돌고 있었다 — 모든 단말에 대한 실시간
  push가 조용히 아무 일도 안 하고 있었다는 뜻이다. 크게 티가 나진
  않았다: 화면마다 자기 폴링(15초)이 있어서 시스템은 정상으로
  보였고, 그냥 살짝 느리게 느껴질 뿐이었다. 새 주문을 하나 넣어보고
  Device Server 로그에 그 주문에 대한 줄이 하나도 없다는 걸로 잡았다.
  `docker-start-all.sh`가 처음부터 이 플래그를 안 넣었고, Order를
  재배포할 때마다(이번 주 안에 이 프로젝트에서 두어 번 있었다) 같은
  `docker run` 줄을 복사하면서 그 공백이 계속 이어졌다. 서버와
  스크립트 둘 다 고쳤고, Order 저장소 README와 Debian-Setting의
  docker.md에 경고로 남겼다. 남길 만한 일반적인 교훈: "지난번 명령을
  복사"하는 재배포는 지난번에 빠졌던 것도 그대로 이어간다 — 직전
  실행 명령과 대조하기보다 그 서비스가 문서화한 환경변수
  (`application.yml`의 기본값)와 대조하는 쪽이 안전하다.
- **무엇, 2026-09-13**: 메뉴별 KDS 라우팅을 끝단까지 연결했다 — 메뉴
  하나를 특정 KDS 스테이션에 지정하면, 그 메뉴의 수락된 주문은 매장의
  모든 KDS가 아니라 그 KDS 하나에만 push된다. 저장소 넷을 건드렸다:
  `kdsDeviceId`는 `menuName`처럼 Order/OrderEvent에 그냥 얹힌다(마이그레이션
  없음); 실제로 그 값을 정하는 건 주문 채널 자신의 카탈로그이고, 메커니즘을
  증명하려고 데모 스테이션 두 개로 나눴다; Device Server는 registry의
  기존 `channelFor`로 단말 하나를 직접 겨냥한다(전체 broadcast 대신); BO의
  메뉴 화면에도 같은 필드를 넣었지만 표시용이다 — BO 메뉴 마스터가 진짜
  카탈로그가 되는 날을 위한 것이고 지금 실제 주문에는 안 물려 있다.

  **실물로 확인하다가 진짜 버그 두 개를 잡았다:**
  (1) 소유자가 스크린샷 하나로 첫 번째를 잡아줬다: `TerminalId`가
  (매장, 장비ID)로만 키가 잡혀 있고 단말 종류가 없어서, 한 매장의 POS와
  KDS를 똑같이 장비ID "01"로 열면 서로 밀어냈다 — storeId 자체를 추가했던
  그 이유와 정확히 같은 버그가 한 단계 위에서 그대로 있었던 것이다.
  identity에 `type`을 추가해서 고쳤고, 아래의 다중매장 DID도 이 김에
  storeId 자체를 identity에서 뺄 필요가 생겼다 — 여러 매장을 보는 화면은
  어느 한 매장 "소속"이 아니기 때문이다.
  (2) KDS 탭 두 개를 나란히 열어보다가 두 번째를 잡았다: 스테이션별
  push는 됐는데, 각 KDS의 목록 조회(`GET /orders`)는 서버 쪽 필터가
  없어서 15초 폴링이나 새로고침이 모든 스테이션 주문을 모든 화면에
  다시 보여줬다 — push만 고치고 폴링 쪽을 안 고치면 고친 게 아니라
  문제가 드러나는 시점만 늦춘 것일 뿐이다.
- **무엇, 2026-09-13**: "DID도 층별로 나누면"이라는 곁가지 아이디어에서
  출발해서, 소유자가 더 쓸모 있는 방향을 제안했다: 한 매장의 DID를
  구역별로 쪼개는 대신, **DID 하나가 여러 매장을 동시에 보는 것** —
  푸드코트 스타일이다. handshake의 `storeId`가 DID에 한해서 쉼표로
  구분된 목록일 수 있게 됐다(POS/KDS는 여전히 정확히 하나만 허용 — 카운터는
  물리적으로 한 매장에 있으니까). Device Server는 그 연결을 나열된 모든
  매장 각각의 DID 그룹에 등록해두고, **기존의 매장별 broadcast가 그대로
  거기까지 닿는다** — push 경로는 손댈 게 없었다. DID 탭 하나가 두 매장의
  준비완료 주문을 동시에, 각각 라벨을 달고 보여주는 것과, 지켜보지 않는
  세 번째 매장의 주문은 끝까지 안 뜨는 것까지 실물로 확인했다.
- **무엇, 2026-09-13**: 소유자가 "POS에 일자 관리 부분이 왜 없냐"고
  물었고, 답은 정말 없었다는 것이었다 — 개점/마감(하루 영업 열고 닫기)은
  2026-09-10부터 백엔드에만 있던 기능이었고(Order 자신의 endpoint,
  umbrella `docs/adr/0006`), 배포 서버의 매장 10개는 전부 지금까지
  Order에 직접 curl을 날려서만 열려 있었다. 버튼을 아무도 안 만든 것이다.
  단말이 다른 걸 닿는 것과 똑같은 방식으로 Device Server를 거치게 했고
  (`GET /business-days/status`, `POST /business-days/{open,close}`, body는
  파싱해서 다시 만들지 않고 Order에 그대로 넘긴다), POS 헤더 아래에 상태
  바와 버튼 두 개를 붙였다. curl이 아니라 **실제 브라우저에서 마감을
  누르고 다시 개점을 눌러서** 화면 자체의 상태가 바뀌는 것까지 확인했다
  — API 레벨에서 curl로 이미 증명했던 것과 같은 왕복을, 이번엔 화면
  클릭으로.
- **무엇, 2026-09-15**: BO에 없던 운영 화면 셋을 붙였다 — 매출 조회
  (일자별), 단말 현황, 운영자 관리. 앞의 둘은 **아무것도 소유하지
  않는다**: 매출은 Order에서, 접속 현황은 Device Server에서 매번 읽어
  온다. BO가 사본을 들고 있으면 몇 분 지난 상태를 자신 있게 보여주게
  되고, 그건 이 화면들이 존재하는 이유 자체를 없앤다. 그래서 조회만 있고
  고치는 화면은 없다 — 관리 화면에서 매출을 바꿀 수 있게 되는 순간 어느
  쪽이 사실인지 말할 수 없게 된다. 죽은 업스트림은 빈 표가 아니라 어느
  service인지 이름을 붙인 502로 보인다: "어제 매출이 0원"과 "매출을
  물어볼 수 없었다"는 다른 사실이고, 이 계열은 이미 조용히 꺼져 있던
  이벤트 publisher 때문에 한 시간을 엉뚱한 데서 찾은 적이 있다.
- **설계가 화면이 되는 지점, 2026-09-15**: 단말 현황은 제어면과 실행면이
  한 화면에 같이 보이는 첫 자리다. BO는 무엇이 있어야 하는지를 정하고,
  Device Server는 무엇이 실제로 붙어 있는지를 안다. 두 쪽이 어긋나는
  두 방향이 곧 화면의 내용이다 — 등록됐는데 접속 없음(카운터에 화면이
  안 떠 있다, 그 매장은 주문을 못 받는다)과, 접속했는데 등록 없음(장비
  목록에 없는 단말기번호로 붙었다). 두 번째는 Device Server가 아직
  형식만 확인하고 BO 목록과 대조하지 않아서 생기는 것이고, 지금은 이
  줄이 그 사실을 볼 수 있는 유일한 자리다. 실물로 양방향 다 확인했다:
  POS 한 대를 브라우저로 띄워 `connected:true`가 뜨는 것과, 등록 안 된
  `kds-99`로 붙어서 `registered:false`가 뜨는 것.
- **화면을 만들다가 드러난 것, 2026-09-15**: BO의 장비 마스터에 **매장이
  없었다.** Device Server는 단말을 (매장, 단말기번호, 종류)로 식별하는데
  — 지점마다 `pos-01`이 있으니까 — BO 쪽에는 매장이 없어서 등록된 단말과
  접속 중인 단말을 맞출 방법이 아예 없었다. 조인을 짜는 순간 드러났다.
  설계의 구멍은 그 위에 다음 것을 올리기 전까지 구멍처럼 보이지 않는다.
  같은 날 `WebConfig`에 `/menus`와 `/stores` 줄이 빠져 있던 것도 찾았다
  — 메뉴로 눌러 들어가면 되지만 그 상태로 새로고침하면 404였다. 닷새
  동안 아무도 보고하지 않았는데, 눌러서 들어가는 경로는 멀쩡했기
  때문이다.
- **운영자 관리, 2026-09-15**: 세 단계 권한 모델은 BO의 첫 커밋부터
  강제되고 있었지만, 권한을 주는 유일한 방법이 손으로 INSERT 하는 것이라
  배포 서버 계정은 지금까지 딱 하나였다. 화면 × 행위 매트릭스를 붙이면서
  관리 콘솔이 스스로를 잠가버리는 두 경우를 서비스 쪽에서 막았다:
  마지막 전체 관리자는 권한을 내리거나 사용 중지하거나 삭제할 수 없고
  (초기 계정 seed는 운영자 테이블이 *비어 있을 때만* 동작하므로, 일반
  계정만 남은 테이블은 DB에 직접 손대야 복구된다), 본인 계정도 삭제할 수
  없다 — 세션이 행보다 오래 살아남아서 그 뒤의 모든 클릭이 BO가 고장난
  것처럼 보이기 때문이다.
- **무엇, 2026-09-15 (같은 날 후반)**: 소유자가 두 가지를 짚었다.
  KDS·DID가 **판매일자를 전혀 모른다**는 것과, 주문 채널이 **메뉴를
  누르면 곧바로 주문된다**는 것. 둘 다 재료는 이미 있었다 — 주문마다
  판매일자가 찍혀 내려오는데 두 단말의 `Order` interface에 그 필드가
  없어서 받고도 버리고 있었고, 영업일을 물어볼 창구도 POS가 쓰는 Device
  Server 경유 endpoint로 이미 열려 있었다. 필터는 **상태로 들고 있지
  않고 파생**시켰다: 그래야 "초기화"라는 별도 동작이 없고, 따라서 그것을
  실행하는 것을 잊을 자리도 없다. DID는 매장별로 따로 판단한다 —
  푸드코트는 한 덩어리로 마감하지 않으니까.
- **소유자가 화면을 보고 찾은 버그, 2026-09-15**: "DID가 왜 과거 주문이
  다시 보였다가 사라지고 다시 보이고 사라지죠?" 실제 루프였다. 15초가
  끝나면 카드를 Map에서 지우는데, 그 주문은 서버에서 여전히 PRODUCED이고
  (딜리버리 채널이 없어 그 다음으로 넘어가지 않는다), 다음 폴링이 그것을
  새 주문처럼 다시 집어와 또 15초를 준다. 하루 종일. README에는 "새로
  고치면 다시 15초 보인다"는 한계만 적혀 있었는데, 같은 구멍이 **새로
  고침 없이도** 계속 재생하고 있었다. 이미 창을 다 쓴 주문 id를 기억해
  건너뛰게 고쳤다 — 들어올 때가 아니라 **나갈 때** 기록하므로, 화면에
  떠 있는 채로 탭이 닫히면 다시 한 번 받고 이미 다 본 것은 영영 안
  돌아온다. 근본 해결은 위쪽이다: 딜리버리 채널이 생겨 주문이 PRODUCED를
  벗어나면 서버가 같은 주문을 계속 건네주지 않게 된다.
- **무엇, 2026-09-15**: 주문 채널에 확인 단계를 넣었다. 질문 자체보다
  **모양**이 요점이다 — 누름과 주문 사이의 단계를 배열(`FLOW`)로
  선언해서, 단계를 하나 늘리는 일이 배열에 이름 하나 + 렌더러에 case
  하나가 되게 했다. 결제가 거기로 들어간다. 주문을 보내고 결과를 보이는
  것은 일부러 그 목록 밖에 뒀다: 손님이 하는 일이 아니고, 섞어 놓으면 새
  단계를 넣을 때마다 "주문이 이미 나간 뒤에 오지 않도록" 조심해야 한다.
- **무엇, 2026-09-15**: Java 쪽 큰 틀이 끝나고 소유자가 직접 손대기
  시작하는 시점이라, 개발 환경 문서를 따로 만들었다
  (`Debian-Setting/docs/development.md`). 배포 문서의 앞 단계 — 어디에
  앉아서 무슨 도구로 고치고, 고친 것이 어느 경로로 서버까지 오는가.
  갈림길은 사실상 두 가지 사실이 정한다: **서버에 Node가 없고, 작업
  PC에 Docker가 없다.** 그래서 Spring service는 push 후 서버에서 docker
  build이고(화면이 jar 안에 있어도 마찬가지), 단말 화면은 PC에서 npm
