...
기술적 결정은 기술 저장소의 ADR에 기록하고, 이 저장소는 커리어 결정과 그 근거를 기록하기 위해 작성되었다.

결국 이 저장소는 기술적 성장과 커리어 성장의 과정을 함께 정리하고 추적하기 위한 기록 공간이다.

## 기술 저장소와의 관계

기술 저장소는 시스템이다. 코드, 테스트, Architecture와 관련된 기술적 의사결정을 ADR로 관리한다.
이 저장소는 그 시스템을 만드는 방향성이다. 왜 해당 시스템을 선택했는지, 왜 특정 기술을 학습하는지, 왜 이 시점에 해당 직무를 목표로 하는지를 기록한다.

두 저장소를 분리한 이유는 목적이 다르기 때문이다. 기술 저장소는 엔지니어링 결과물을 설명하고, 이 저장소는 커리어 전략과 방향성의 과정을 설명한다.
