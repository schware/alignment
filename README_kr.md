...
이 저장소는 그 시스템을 만드는 사람이다. 왜 해당 시스템을 선택했는지, 왜 특정 기술을 학습하는지, 왜 이 시점에 해당 직무를 목표로 하는지를 기록한다.

두 저장소를 분리한 이유는 목적이 다르기 때문이다. 기술 저장소는 엔지니어링 결과물을 설명하고, 이 저장소는 커리어 전략과 성장 과정을 설명한다.
